    ...
    internal class DownloadAccelerator
    {
        public class Downloader
        {
            ...
            public int RemainingTimeBySeconds(DateTime StartingTime, Chunk chunk)
            {
                ...
                // Remaining time = (Remaining Bytes) / (Bytes per second)

                int remaining = (int)(bytesRemaining / bytesPerSecond);


                return remaining > 60 ? remaining / 60 : remaining;
            }
            ...
            public async Task start(IProgress<Chunk> ProgressCallBack)
            {
                ...
                if (!CreateSavingFile())
                {
                    throw new Exception("cannot create the file in your machine ,try running the app as a admin or check your storage space");
                }
