...
{
    public partial class FilesDecryptorFrm: Form
    {
        public FilesDecryptorFrm()
        {
            InitializeComponent();
        ...
        private void StartDecryption()
        {
            ...
            for (int i = 0; i < dataGrid1.Files.Count; i++)
            {
                ...
                Task.Run(() =>
                {
                    if (FileEncryptor.DecryptFile(key, dataGrid1.Files[index], outdirectory + @"\" +"Decrypted "+ Path.GetFileName(dataGrid1.Files[index])))
